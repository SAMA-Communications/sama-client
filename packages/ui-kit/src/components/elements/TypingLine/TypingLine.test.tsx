...
vi.mock("../../DotsLoader", () => ({
  DotsLoader: ({ height, width }: any) => (
    <div data-testid="dots-loader" data-height={height} data-width={width} />
  ),
}));
...
describe("TypingLine", () => {
  ...
  it("renders two usernames correctly", () => {
    ...
    expect(
      screen.getByText("FirstName1, FirstName2 typing")
    ).toBeInTheDocument();
  });

  it("renders 'and N more' when more than two users", () => {
    render(
      <TypingLine typingUserIds={["u1", "u2", "u3"]} isDisplayUserNames />
    );
    expect(
      screen.getByText("FirstName1 and 2 more typing")
    ).toBeInTheDocument();
  });

  it("applies background class when isDisplayBackground = true", () => {
    const { container } = render(
      <TypingLine typingUserIds={["u1"]} isDisplayBackground />
    );
    const wrapper = container.querySelector("div");
    expect(wrapper).toHaveClass("bg-accent-dark/10");
  });
});
