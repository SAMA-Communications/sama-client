...
export default function AttachHub() {
  ...
  const addFiles = async (newFiles) => {
    ...
    try {
      ...
      err.message ? showCustomAlert(err.message, "warning") : console.error(err);
      setIsPending(false);
      return;
    }
  ...
  const sendMessage = useCallback(
    async (event) => {
      ...
      if (body?.length === 0 && !files.length) return;
  ...
  const handleInput = (e) => {
    if (inputTextRef.current) {
      ...
      inputTextRef.current.style.height = `calc(${40 + countOfLines * 20 < 230 ? 40 + countOfLines * 20 : 215}px`;
  ...
  useEffect(() => {
    ...
    function handleInput(e) {
      ...
      if (clipboardItems?.items) {
        fileList = extractFilesFromClipboard(clipboardItems);
